...
```{r examples-uj_matx_v0, eval = FALSE}
#' \dontrun{
...
test_that("test uj_matx", {
  ...
  expect_equal(uj_matx_v0(max.val = 100, ratio.util = 0.1, ratio.just = 0.1),
               mat_test)
  
  expect_error(uj_matx_v0(max.val = 0))
  expect_error(uj_matx_v0(max.val = 'A'))
  
  expect_error(uj_matx_v0(ratio.util = 0.6))
  expect_error(uj_matx_v0(ratio.util = 0))
  
  expect_error(uj_matx_v0(just.util = 0.6))
  expect_error(uj_matx_v0(just.util = 0))
...
#' }
#' @source simK::candidates_df(n = 2000, prob.urgent = 0)
"cds"
```
